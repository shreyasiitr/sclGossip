  ...
  public void run() {
    while (keepRunning.get()) {
      try {
        ...
        ActiveGossipMessage activeGossipMessage = (ActiveGossipMessage)o;
        List<com.google.code.gossip.model.GossipMember>memList = activeGossipMessage.getMembers();
        //System.out.println("element : "+memList.get(0).getHeartbeat());
        //
        int packet_length = 0;
        for (int i = 0; i < 4; i++) {
          int shift = (4 - 1 - i) * 8;
...
//          }
          try {
        	  GossipMember me = gossipManager.getMyself();
...
//                    ActiveGossipMessage.class);
            for (int i = 0; i < activeGossipMessage.getMembers().size(); i++) {
              RemoteGossipMember member = new RemoteGossipMember(
                      ...
                      activeGossipMessage.getMembers().get(i).getLogInstance(),
                      activeGossipMessage.getMembers().get(i).getHeartbeat());
              //member.setLogInstance(activeGossipMessage.getMembers().get(i).getLogInstance());
              //System.out.println("Received log size : "+activeGossipMessage.getMembers().get(i).getLogInstance().size());
              me.merge(member.getLogInstance());
              if (!(member.getClusterName().equals(cluster))){
                GossipService.LOGGER.warn("Note a member of this cluster " + i);
                continue;
              }
              //System.out.println("My id : "+me.getId()+" total entries : "+me.getLogInstance().size());
            ...
            if(me.getId().equals("1")) {
            	//System.out.println("Log Instance size : "+me.getLogInstance().size());
            }
            //System.out.println("Total members : "+gossipManager.getMemberList().size());
